...
void __fastcall TForm3::DoFindText()
{
    TPoint CaretPos;
    int x, y, i, l;
    AnsiString FindStr;
    ...
    CaretPos = Form3->Memo1->CaretPos;
    x = CaretPos.x;
    y = CaretPos.y;

    FindStr = Form3->FindDialog1->FindText;
    if (FindStr.IsEmpty()) return;

	SFind = FindStr.c_str();
//  Здесь ошибка - возвращает тип const char*
//    SStr = Form3->Memo1->Lines->Text.c_str();

// Здесь с C++ Builder 10.x возвращает wchar_t*
	SStr = AnsiString(Memo1->Lines->Text).c_str();


    // Вычисляем позицию курсора в общем тексте
    l = x;
    for (i = 0; i < y; i++)
        l += Form3->Memo1->Lines->Strings[i].Length() + 2;

    if (Form3->FindDialog1->Options.Contains(frDown))
    {
        for (i = l + 1; i < Form3->Memo1->Lines->Text.Length(); i++)
        {
            if (strncmp(SStr + i, SFind, FindStr.Length()) == 0)
            {
                Form3->Memo1->SetFocus();
                Form3->Memo1->SelStart = i;
                Form3->Memo1->SelLength = FindStr.Length();
                return;
            }
        }
    }
    ...
    {
        for (i = l - 1 - FindStr.Length(); i >= 0; i--)
        {
            if (strncmp(SStr + i, SFind, FindStr.Length()) == 0)
            {
                Form3->Memo1->SetFocus();
