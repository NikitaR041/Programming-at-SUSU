...
//          c->TextOut(Rect.Left + (Rect.Right-Rect.Left)/2 - 6, Rect.Top + (Rect.Bottom-Rect.Top)/2 - 8, "K");
			break;
		case BLACK_KING:
			c->Brush->Color = clBlack;
			c->Ellipse(Rect.Left + 5, Rect.Top + 5, Rect.Right - 5, Rect.Bottom - 5);
...
// Ход со взятием (перескок через 1 фигуру)
...
// Проверка: с данной клетки возможен ли любой прыжок
...
// Проверка: есть ли у текущего игрока вообще возможность бить
...
// Выполнение хода; если сделан прыжок — удаляет взятую шашку.
// После выполнения: если прыжок был и возможны дальнейшие прыжки — не меняем игрока.
// Иначе — выполняем switchPlayer().
...
// Превращение в дамку (русские шашки: белые на y==0, черные на y==7)
...
void TForm3::switchPlayer() {
	if (currentPlayer == WHITE_PAWN) currentPlayer = BLACK_PAWN;
	else currentPlayer = WHITE_PAWN;
}
...
void __fastcall TForm3::DrawGrid1MouseDown(TObject *Sender, TMouseButton Button, TShiftState Shift,
          int X, int Y)
{
    // Вычисляем индексы колонки/строки по координатам
	...
	if (!isSelected) {
		...
		if (isValideChecker(xCell, yCell)) {
            // Если существует обязательное взятие у любого шашки текущего игрока,
            // запрещаем выбор шашки, у которой нет взятия (правило обязательного взятия)
    ...
    if (isSelected) {
        ...
        if (anyCaptureExists) {
            // Нельзя делать простой ход — игнорируем
            isSelected = false;
            DrawGrid1->Repaint();
            return;
        }
