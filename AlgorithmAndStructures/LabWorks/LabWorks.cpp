...
//Для хранение неориентированного графа подойдет список(матрица) смежностей.

class Graph {
private:
    int V; //Количество вершин
    int E; //Количество рёбер
    std::vector<std::pair<int, int>> edges; // Список пар рёбер, каждое ребро (u, v) хранится один раз
    std::vector<std::unordered_set<int>> adjacencyList; // Список смежности: adjacencyList[v] хранит список соседей вершины v
public:
    //Требование: нет.
    //Действие: Увеличивает количество вершин на 1. Возвращает номер добавленный вершины.
